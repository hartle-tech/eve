...
#
# Hand-rolled rather than driven by the Tauri CLI, so `cargo` and a stock
# macOS are the only requirements *to assemble it* — the built frontend is
# committed in ui/, so npm is needed only to change the interface:
#
#   cd ui-src && npm install && npm run build The result is a single droppable
...
printf '\nSigning\n'
# --timestamp and --options runtime only with a real identity. The hardened
# runtime is what notarisation requires, and a secure timestamp is what keeps
# the signature valid after the certificate expires. Neither is meaningful
# ad-hoc, and --timestamp there just fails against Apple's timestamp server.
HARDENED=()
if [[ "$IDENTITY" != "-" ]]; then
  HARDENED=(--timestamp --options runtime)
fi
codesign --force --deep --sign "$IDENTITY" "${HARDENED[@]}" "$APP" 2>&1 | sed 's/^/  /' \
  || die "codesign failed"
