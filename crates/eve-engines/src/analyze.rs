...
pub fn largest_files(root: &Path, limit: usize, budget: Duration) -> Vec<Entry> {
        ...
        .filter_map(std::result::Result::ok)
    {
        ...
        h.push(Entry {
            ...
            bytes: eve_core::size::on_disk(&meta),
            files: 1,
            is_dir: false,
            cleanable: None,
        });
...
pub fn by_extension(root: &Path, budget: Duration) -> Vec<(String, u64, u64)> {
        ...
        .filter_map(std::result::Result::ok)
    {
        ...
        slot.0 += eve_core::size::on_disk(&meta);
        slot.1 += 1;
    }
...
mod tests {
    ...
    fn ranks_children_by_size() {
        ...
        assert!(a.total_bytes >= 10_100, "got {}", a.total_bytes);
    }
    ...
    fn flags_cleanable_directories() {
        ...
        assert!(a.cleanable_bytes() >= 50, "got {}", a.cleanable_bytes());
    }
    ...
    fn largest_files_are_ordered_and_capped() {
        ...
        assert_eq!(top.len(), 2);
        assert!(top[0].bytes >= 5000, "allocated size rounds up: {}", top[0].bytes);
        assert!(top[1].bytes >= 300, "allocated size rounds up: {}", top[1].bytes);
    }
    ...
    fn extension_totals_add_up() {
        ...
        // Allocated blocks, so two small files occupy a block each.
        assert!(log.1 >= 300, "got {}", log.1);
        assert_eq!(log.2, 2);
    }
