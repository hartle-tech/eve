...
fn print_category(cat: &CategoryResult, style: &Style, verbose: bool) {
    ...
    // Permitted, attempted, and did not happen. Louder than a refusal, because
    // a refusal is eve working as designed and this is not.
    for failure in cat.execution_failures() {
        println!("  {} {}", style.yellow("✗"), style.yellow(&failure));
    }
}
