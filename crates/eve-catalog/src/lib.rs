...
pub fn catalog_for(home: impl Into<PathBuf>) -> Vec<Category> {
    ...
    add(
        "trash",
        "Trash",
        "Permanently delete what is in the Trash. Irreversible, so it is off \
         until you turn it on — and once on it stays on, because the run that \
         fills the Trash is usually the unattended one. Runs before everything \
         else, so this run's own deletions still land in an emptied Trash and \
         stay recoverable until the next run.",
        Group::User,
        RiskTier::Review,
        RunContext::User,
        // Not `Permanent`: that removes `~/.Trash` itself, which macOS expects
        // to exist, rather than emptying it.
        Disposition::PermanentContents,
        vec![b.h(".Trash")],
        vec![],
        vec![],
        vec![],
    ...
    add(
        "user_logs",
        "User logs",
        "~/Library/Logs. Diagnostic reports are left alone — see below.",
        Group::User,
        RiskTier::Safe,
        RunContext::User,
        Disposition::EmptyContents,
        // `~/Library/Logs` only.
        //
        // This used to also take `~/Library/Application Support/CrashReporter`,
        // and that one line cost a system permission dialog on every launch:
        // anything under another app's Application Support is an *App Data*
        // access, and macOS asks about those. It was worth two plists.
        vec![b.h("Library/Logs")],
        vec![],
        vec![],
        vec![],
    );
    ...
    add(
        ...
        "Cache directories inside ~/Library/Containers and Group Containers. \
         Off until you turn it on, because reading them is what makes macOS \
         ask whether eve may see other apps' data — and on this machine the \
         whole category comes to about a megabyte, most of it refused anyway \
         because the owning apps are running.",
        Group::AppCaches,
        // Not `Safe`, despite being caches.
        //
        // The tier here is not about the risk of deleting them; it is about
        // the cost of *looking*. Walking ~/Library/Containers is the access
        // macOS guards with the App Data prompt, and as a default-on category
        // it raised that prompt on every launch, for ever, to reclaim 1.3 MB.
        // A permission dialog on every launch is a real cost, and it should
        // buy more than a megabyte.
        RiskTier::Review,
        RunContext::User,
        Disposition::EmptyContents,
    ...
    add(
        "electron_caches",
        "Electron app caches",
        "Cache / Code Cache / GPUCache / ShaderCache inside Electron-style \
         apps. Off until you turn it on: these live in ~/Library/Application \
         Support, so looking at all is what makes macOS ask whether eve may \
         see other apps' data.",
        Group::AppCaches,
        // See `container_caches`: the tier is about the cost of *looking*.
        // Walking ~/Library/Application Support is an App Data access, and as
        // a default-on category it meant every launch raised a system
        // permission dialog. This whole family comes to about 25 MB on this
        // machine, against 9.9 GB of Rust build output that needs no such
        // permission — it is not a trade worth making by default.
        RiskTier::Review,
        RunContext::User,
        Disposition::Trash,
    ...
    add(
        "rust_target",
        "Rust build output",
        "target/ directories beside a Cargo.toml. Usually the largest thing on \
         a Rust machine, and rebuilt by the next cargo build.",
        Group::Projects,
        RiskTier::Safe,
        RunContext::User,
        Disposition::Trash,
        vec![Target::Dynamic(dynamic::Kind::RustTargets)],
        vec![],
    ...
    add(
        ...
        ".venv and venv directories holding a pyvenv.cfg. Off until you turn it \
         on: recreating one needs the network, and a venv can hold packages \
         installed from somewhere you no longer remember.",
        Group::Projects,
        RiskTier::Review,
        RunContext::User,
        Disposition::Trash,
        vec![Target::Dynamic(dynamic::Kind::PythonVenvs)],
        vec![],
        vec![home.join("Projects")],
        vec![],
    );
    ...
    add(
        "ide_caches",
        "Editor caches",
        "VS Code, VSCodium and Eclipse workspace state and indexes. Rebuilt on \
         next open. JetBrains is left alone — re-indexing costs hours there.",
        Group::Developer,
        RiskTier::Safe,
        RunContext::User,
        Disposition::EmptyContents,
        vec![Target::Dynamic(dynamic::Kind::IdeCaches)],
        vec![],
        vec![],
        vec![],
    );
    ...
    add(
        ...
        "Cached sandbox VM bundles. Regenerate on demand and are frequently \
         the single largest item on the disk — but they live inside Claude's \
         own data, so turning this on is what makes macOS ask whether eve may \
         look there. It asks once per run, not once per launch.",
        Group::Developer,
        // Opt-in, and reluctantly: this is often the biggest single reclaim
        // available. It is the only remaining default path into another app's
        // data, and leaving it on meant a permission dialog every time eve
        // opened — which is a worse deal than asking the user to switch it on
        // when they want the space.
        RiskTier::Review,
        RunContext::User,
        Disposition::Trash,
