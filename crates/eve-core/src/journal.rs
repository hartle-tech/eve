...
impl JournalEntry {
    ...
    pub fn recoverable(&self) -> bool {
        !self.dry_run && self.error.is_none() && self.disposition.is_recoverable()
    }
}
...
impl Journal {
    pub fn open_default() -> Result<Self> {
        // One source of truth for where eve's own state lives. This used to
        // carry its own copy of the path, so the journal and the preferences
        // could disagree — and both pointed at a directory named for neither
        // eve's bundle id nor anything else macOS recognises, which is what
        // made opening the journal an "access to data from other apps".
        let base = crate::prefs::state_dir();
    ...
    /// Empty the history.
    ///
    /// The file is truncated rather than deleted, so the journal stays open and
    /// the next write does not have to recreate it mid-run. Returns how many
    /// entries went, because "cleared" with no number is indistinguishable from
    /// "did nothing".
    pub fn clear(&self) -> Result<usize> {
        let had = self.read_all().map(|v| v.len()).unwrap_or(0);
